...
class simple_html_dom
{
    ...
    function loadFile() {$args = func_get_args();$this->load_file($args);}
		...
		function parseItem()
		{
				$dom = new simple_html_dom_node($this);
		    return [
		        'title' => function () use ($dom) {
		            return $this->find('.dbg0pd')->firstChild->getNodeValue();
		        }
		        /*
		        ,
		        'url' => function () use ($localPack, $dom) {
		            ...
		            $nodes = $this->cssQuery('a.L48Cpd', $localPack);
		            if ($nodes->length > 0) {
		                return $nodes->getNodeAt(0)->getAttribute('href');
		            } else {
		                return $this->cssQuery('link[href]', $localPack)
		                    ->getNodeAt(0)
		                    ->getAttribute('href');
		            }
		        },

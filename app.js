...
am4core.ready(function() {
  ...
  var recoveredColor = am4core.color("#00FFFF");
  var deathsColor = am4core.color("#1c5fe5");
	var testsColor = am4core.color("#00ff11");
	var criticalColor = am4core.color("#FF0080");

  // for an easier access by key
  var colors = { active: activeColor, confirmed: confirmedColor, recovered: recoveredColor, deaths: deathsColor, tests: testsColor, critical: criticalColor  };
  ...
  var currentIndex;
	var currentCountry = "Greece (Total)";
	
	var covid_us_timeline = JSON.parse(getJSON('https://raw.githubusercontent.com/Sandbird/covid19-Greece/master/regions.json'));
	var covid_us_total_timeline = JSON.parse(getJSON('https://raw.githubusercontent.com/Sandbird/covid19-Greece/master/all.json'));	
  ...
  /*
  for (var i = 0; i < covid_us_total_timeline.length; i++) {
  ...
  */
  ...
  var max = { confirmed: 0, active: 0, deaths: 0, recovered: 0, tests: 0, critical: 0};
  //var maxPC = { confirmed: 0, active: 0, recovered: 0, deaths: 0, active: 0 };
  ...
  mapChart.geodata = am4geodata_greeceLow;
  ...
  title.fontSize = "1.5em";
	title.text = "COVID-19 GR Spread Data";
  title.align = "left";
  ...
  var deathsSeries = addSeries("deaths", deathsColor);
	var testSeries = addSeries("tests", testsColor);
	var criticalSeries = addSeries("critical", criticalColor);

  var series = { active: activeSeries, confirmed: confirmedSeries, deaths: deathsSeries, recovered: recoveredSeries, tests: testSeries, critical: criticalSeries};
  // add series
  ...
  function createColumnSeries() {
    ...
    columnSeries.recovered = addColumnSeries("recovered", recoveredColor);
		columnSeries.tests = addColumnSeries("tests", testsColor);
		columnSeries.critical = addColumnSeries("critical", criticalColor);
  }
  ...
  var buttons = { active: activeButton, confirmed: confirmedButton, deaths: deathsButton, recovered: recoveredButton, tests: testsButton, critical: criticalButton};
  ...
  function setCountryData(countryIndex) {
    ...
    for (var i = 0; i < lineChart.data.length; i++) {
      ...
      if (countryData) {
				...
				dataContext.tests = countryData.tests;
				dataContext.critical = countryData.critical;
  ...
  function showWorld() {
    ...
    for (var i = 0; i < lineChart.data.length; i++) {
      ...
      dataContext.tests = di.tests;
      dataContext.critical = di.critical;
      valueAxis.min = undefined;
      valueAxis.max = undefined;
    }
  ...
  function updateTotals(index) {
	...
	function updateMapData(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (polygon) {
				polygon.dataItem.dataContext.confirmedPC = di.confirmed / population * 100000;
				polygon.dataItem.dataContext.deathsPC = di.deaths / population * 100000;
  ...
  function idToName(id) {
		return am4geodata_data_states_en[id] ? am4geodata_data_states_en[id] : id;
  }
